...
runWhenDomReady(async () => {
  ...
  const gamePilotBtn = document.getElementById("gamePilotBtn");
  ...
  const iconSvg = {
    ...
    gamepad: '<path d="M17.32 5H6.68a4 4 0 0 0-3.98 3.59l-.64 6A3 3 0 0 0 5.05 18h.35a3 3 0 0 0 2.12-.88L9.64 15h4.72l2.12 2.12a3 3 0 0 0 2.12.88h.35a3 3 0 0 0 2.99-3.41l-.64-6A4 4 0 0 0 17.32 5Z"/><path d="M6 11h4"/><path d="M8 9v4"/><path d="M15 12h.01"/><path d="M18 10h.01"/>',
  ...
  const applyState = (state = {}) => {
    ...
    setActive(gamePilotBtn, state.gamePilotEnabled);
    if (gamePilotBtn) {
      gamePilotBtn.title = state.gamePilotEnabled
        ? "AI game pilot preview is on"
        : "AI game pilot preview";
    }
  ...
  clearChatBtn?.addEventListener("click", () => emitCommand("chat_clear"));
  ...
  gamePilotBtn?.addEventListener("click", () => emitCommand("game_pilot_toggle"));
